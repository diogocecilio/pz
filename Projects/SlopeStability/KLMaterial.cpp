...
KLMaterial::KLMaterial ( int id,REAL Lx,REAL Ly, int type,int expansionorder )
{
  fId=id;
  fLx=Lx;
  fLy=Ly;
...
KLMaterial::KLMaterial ( const KLMaterial &mat ) :fId ( mat.fId ),fLx ( mat.fLx ),fLy ( mat.fLy ),ftype ( mat.ftype )
{
  SetId ( mat.fId );
}
...
{
  REAL val = 0, xx1, xx2, yy1, yy2, dist;
