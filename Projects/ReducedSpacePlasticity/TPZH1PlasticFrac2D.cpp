//
//  TPZH1PlasticFrac2D.cpp
//  PZ
//
//  Created by Nathan Shauer on 07/01/14.
//
//
...
TPZH1PlasticFrac2D<T,TMEM>::TPZH1PlasticFrac2D() : TPZMatElastoPlastic2D<T,TMEM>()
{
	fmatId = 0;
	fE = 0.;
	fPoiss = 0.;
...
template<class T,class TMEM>
TPZH1PlasticFrac2D<T,TMEM>::TPZH1PlasticFrac2D(int matid, int dim, REAL young, REAL poiss, REAL visc) : TPZMatElastoPlastic2D<T,TMEM>(matid,1)
{
	fmatId = matid;
	fE = young;
	fPoiss = poiss;
...
template<class T,class TMEM>
TPZH1PlasticFrac2D<T,TMEM>::~TPZH1PlasticFrac2D()
{
}
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::Print(std::ostream &out)
{
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::Contribute(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<REAL> &ek, TPZFMatrix<REAL> &ef)
{
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::ContributePlastic(TPZMaterialData &data, REAL weight, TPZFMatrix<REAL> &ek, TPZFMatrix<REAL> &ef)
{
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::ContributePressure(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<REAL> &ek, TPZFMatrix<REAL> &ef)
{
  ...
  int phiuCols = phi_u.Cols();
	...
	if(gState == ECurrentState) //current state (n+1): Matrix stiffnes
	{
		...
		for(int in = 0; in < phipRows; in++)
		{
			...
			ef(phiuRows+in,0) += (-1.) * weight * (w*w*w/(12.*visc)) * dsol_p(0,0) * dphi_p(0,in);
			
			//termo w/deltaT * Vp
			ef(phiuRows+in,0) += (-1.) * weight * w/deltaT * phi_p(in,0);
			
			//termo 2Ql * Vp
			ef(phiuRows+in,0) += (-1.) * weight * (2.*actQl) * phi_p(in,0);
			
			
			//------Matriz tangente-----
			for(int jn = 0; jn < phiuRows; jn++)
			{
				//termo D[ (wˆ3/(12*mi))*gradP * gradVp , w ]
				ek(phiuRows+in, jn) += (+1.) * weight * ( 3.*w*w/(12.*visc) * (2.*phi_u(jn,0)) ) * dsol_p(0,0) * dphi_p(0,in);
				
				//termo D[ w/deltaT * Vp , w ]
				ek(phiuRows+in, jn) += (+1.) * weight * ( 2./deltaT * phi_u(jn,0) ) * phi_p(in,0);
			}
			for(int jn = 0; jn < phipRows; jn++)
			{
				//termo D[ (wˆ3/(12*mi))*gradP * gradVp , p ]
				ek(phiuRows+in, phiuRows+jn) += (+1.) * weight * (w*w*w/(12.*visc)) * dphi_p(0,in) * dphi_p(0,jn);
				
				//termo D[ 2Ql * Vp , p]
				ek(phiuRows+in, phiuRows+jn) += (+1.) * weight * (2.*actdQldp) * phi_p(in,0) * phi_p(jn,0);
			}
		}
	}
	...
	if(gState == ELastState)
	{
		...
		{            
			//termo w/deltaT * Vp
			ef(phiuRows+phip,0) += (+1.) * weight * w/deltaT * phi_p(phip,0);
		}
	}
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::ApplyDirichlet_U(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<> &ek,TPZFMatrix<> &ef,TPZBndCond &bc)
{
	if(gState == ELastState)
	{
		return;
	...
	for(int in = 0 ; in < phr; in++)
	{
		for(int il = 0; il < this->fNumLoadCases; il++)
		{
			...
			dif0 = sol_u[0] - v2[0];
			dif1 = sol_u[1] - v2[1];
			ef(2*in,il) += big * dif0 * phi_u(in,0) * weight;
			ef(2*in+1,il) += big * dif1 * phi_u(in,0) * weight;
			
			/*
			ef(in,il) += big * ( v2(0,il)*phi_u(in,0) + v2(1,il)*phi_u(in,0) ) * weight;
			
			//termo big*u*v da matriz
			ef(in,il) += (-1.)*big * ( phi_u(in,0)*sol_u[0] + phi_u(in,0)*sol_u[1] ) * weight;
			 */
		}
		for (int jn = 0; jn < phr; jn++)
		{
			ek(2*in,2*jn) += big * phi_u(in,0) * phi_u(jn,0) * weight;
			ek(2*in+1,2*jn+1) += big * phi_u(in,0) * phi_u(jn,0) * weight;
			
			//ek(in,jn) += big * ( phi_u(in,0)*phi_u(jn,0) + phi_u(in,0)*phi_u(jn,0) ) * weight;
		}
	}
}

template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::ApplyNeumann_U(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<> &ek, TPZFMatrix<> &ef,TPZBndCond &bc)
{
	if(gState == ELastState)
	{
		return;
	...
	for (int in = 0; in < nr_u; in++)
	{
		//--- residuo ----
		for (int il = 0; il < this->fNumLoadCases; il++)
		{
			
			ef(2*in,0) += weight * ( factor * sol_un * datavec[0].normal[0] * phi_u(in,0) - sol_p[0] * datavec[0].normal[0] * phi_u(in,0) );
			ef(2*in+1,0) += weight * ( factor * sol_un * datavec[0].normal[1] * phi_u(in,0) - sol_p[0] * datavec[0].normal[1] * phi_u(in,0) );
	
			//ef(in,il) += weight * factor * ( phi_u(in,0)*sol_un*datavec[0].normal[0] + phi_u(in,0)*sol_un*datavec[0].normal[1] )
			//- weight * ( phi_u(in,0)*sol_p[0]*datavec[0].normal[0] + phi_u(in,0)*sol_p[0]*datavec[0].normal[1] );
		}
		
		//----- matriz tangente -----
		//termo k(phi_ix*phi_jun*nx + phi_iy*phi_jun*ny)
		for (int jn = 0; jn < nr_u; jn++)
		{
			//REAL phi_jun = phi_u(0,jn);
			
			ek(2*in,2*jn+1) += (-1.) * weight * factor * datavec[0].normal[0] * phi_u(in,0) * phi_u(jn,0);
			ek(2*in+1,2*jn+1) += (-1.) * weight * factor * datavec[0].normal[1] * phi_u(in,0) * phi_u(in,0);
		...
		{	
			ek(2*in,nr_u+jp) += weight * datavec[0].normal[0] * phi_u(in,0) * phi_p(jp,0);
			ek(2*in+1,nr_u+jp) += weight * datavec[0].normal[1] * phi_u(in,0) * phi_p(jp,0);
			
			//ek(in,jp+nr_u) += weight*(phi_u(0,in)*phi_p(jp,0)*datavec[0].normal[0] + phi_u(1,in)*phi_p(jp,0)*datavec[0].normal[1]);				
		}
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::ApplyMixed_U(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<> &ek, TPZFMatrix<> &ef, TPZBndCond &bc)
{
	if(gState == ELastState)
	{
		return;
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::ApplyNeumann_P(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<> &ek, TPZFMatrix<> &ef, TPZBndCond &bc)
{
	if(gState == ELastState)
	{
		return;
	...
	{
		ef(in+r_u,0) += (-1.) * weight * Qinj * phi_p(in,0);
	}
}
...
template<class T,class TMEM>
int TPZH1PlasticFrac2D<T,TMEM>::VariableIndex(const std::string &name){
...
template<class T,class TMEM>
int TPZH1PlasticFrac2D<T,TMEM>::NSolutionVariables(int var){
	if(var == 1) return 1;
	if(var == 2) return 1;
	if(var == 3) return 1;
	if(var == 4) return 1;
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::Solution(TPZVec<TPZMaterialData> &datavec, int var, TPZVec<REAL> &Solout){
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::FillDataRequirements(TPZVec<TPZMaterialData > &datavec)
{
	int nref = datavec.size();
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::FillBoundaryConditionDataRequirement(int type,TPZVec<TPZMaterialData > &datavec)
{
	int nref = datavec.size();
	for(int i = 0; i<nref; i++)
	{
...
template<class T,class TMEM>
void TPZH1PlasticFrac2D<T,TMEM>::SetRunPlasticity(bool IsPlasticity)
{
...
template class TPZH1PlasticFrac2D<TPZSandlerDimaggio<SANDLERDIMAGGIOSTEP2>, TPZElastoPlasticMem>;
template class TPZH1PlasticFrac2D<TPZPlasticStepPV<TPZYCMohrCoulombPV,TPZElasticResponse> , TPZElastoPlasticMem>;
